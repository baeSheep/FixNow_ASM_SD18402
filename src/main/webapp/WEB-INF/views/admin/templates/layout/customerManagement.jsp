<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<h2 class="text-center mb-4" style="color: #00B074;">Quản lý người
	dùng</h2>
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/style/customerManagement.css">
...
<div class="container-fluid mt-5">
	...
	<div class="mb-4 text-center">
		<button class="btn btn-secondary mx-2"
			onclick="showSection('userForm')">Quản Lý Khách Hàng</button>
		<button class="btn btn-secondary mx-2"
			onclick="showSection('providerForm')">Quản Lý Nhà Cung Cấp</button>
	</div>
	...
	<div class="form-section card" id="userForm">
		<div class="card-body">
			...
			<form action="${pageContext.request.contextPath}/management/update"
				method="post">
				<!-- Cập nhật đường dẫn -->
				<div class="row">
					<div class="col-md-6">
						<div class="form-group">
							<label for="userFirstName">Tên</label> <input type="text"
								class="form-control" id="userFirstName" name="firstName"
								placeholder="Nhập tên" required value="${user.firstName}">
						</div>
						<div class="form-group">
							<label for="userLastName">Họ</label> <input type="text"
								class="form-control" id="userLastName" name="lastName"
								placeholder="Nhập họ" required value="${user.lastName}">
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							<label for="userRole">Vai Trò</label> <select
								class="form-control" id="userRole" name="userType" required>
					...
					<div class="col-md-6">
						<div class="form-group">
							<label for="userPhone">Số Điện Thoại</label> <input type="tel"
								class="form-control" id="userPhone" name="phoneNumber"
								placeholder="Nhập số điện thoại" required
								value="${user.phoneNumber}">
						</div>
						<div class="form-group">
							<label for="userPassword">Mật Khẩu</label> <input type="password"
								class="form-control" id="userPassword" name="password"
								placeholder="Nhập mật khẩu" required>
						</div>
						<input type="hidden" name="userID" value="${user.userID}">
						<!-- Đảm bảo rằng ID người dùng được gửi khi cập nhật -->
					</div>
				</div>
				<hr>
			...
			<table class="table table-bordered mt-2" id="userTable">
				...
				<tbody>
					<c:forEach var="user" items="${regularUsers}">
						<!-- Sử dụng regularUsers -->
						<tr>
							...
							<td>
								<form
									action="${pageContext.request.contextPath}/management/delete"
									method="post" style="display: inline;">
									<!-- Xóa người dùng -->
									<input type="hidden" name="id" value="${user.userID}">
									<button type="submit" class="btn btn-danger btn-sm">Xóa</button>
								</form> <a
								href="${pageContext.request.contextPath}/management/edit/${user.userID}"
					...
					<c:forEach var="provider" items="${providers}">
						<tr>
							...
							<td>
								...
								href="${pageContext.request.contextPath}/management/edit/${provider.userID}"
								class="btn btn-warning btn-sm">Chỉnh Sửa</a>
							</td>
						</tr>
					</c:forEach>
